...
- If the allocator assigns a task whose plan is structurally incomplete — its acceptance-bearing children exist but lack dependency edges to the parent — repair it instead of forcing it forward: wire the children with `spice task depends <parent> <child...>`, then release the task with `spice task unclaim` so the allocator can re-route the repaired plan.
