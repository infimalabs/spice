...
- Use `spice task add --project <stem.child>` for public backlog items. Omitting `--project` creates private `agent.*` scratch work. Use `spice task note` for small observations attached to a task.
