...
SHELL_STEER_STARTED_ENV = "SPICE_SHELL_STEER_STARTED"  # env-policy: allow
...
def render_shell_steering_hook(
    *,
    marker_path: Path,
    flush_command: Sequence[str],
    watch_command: Sequence[str],
    ...
    lines = [
        ...
        f'if [ -z "${{{SHELL_STEER_STARTED_ENV}+x}}" ]; then',
        f"  export {SHELL_STEER_STARTED_ENV}=1",
        f"  if [ -s {shell_quote(str(marker_path))} ]; then",
        f"    {shell_command(flush_command)}",
        f"    {shell_command(watch_command)} $$ &",
        "    _spice_steer_pid=$!",
        "    trap 'kill ${_spice_steer_pid} >/dev/null 2>&1; wait ${_spice_steer_pid} >/dev/null 2>&1' EXIT",
        "  fi",
        "fi",
        *restore_lines,
    ]
