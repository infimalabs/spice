...
SPICE_STEER_EMITTED_ENV = "SPICE_STEER_EMITTED"  # env-policy: allow
SPICE_STEER_EMITTED_VALUE = "1"
...
def render_shell_steering_hook(
    *,
    ...
    lines = [
        ...
        (
            f'if [ "${{{SPICE_STEER_EMITTED_ENV}:-}}" != '
            f"{shell_quote(SPICE_STEER_EMITTED_VALUE)} ] && "
            f"[ -s {shell_quote(str(marker_path))} ]; then"
        ),
        f"  export {SPICE_STEER_EMITTED_ENV}={shell_quote(SPICE_STEER_EMITTED_VALUE)}",
        f"  {shell_command(command)} || true",
        "fi",
        *restore_lines,
    ]
