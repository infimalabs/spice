...
def shadow_environment(
    ...
    *,
    ...
    if not branch:
        # Detached HEAD or unborn branch: the shadow self-tracks a branch, so
        # with none it stays inactive and native upstream config is in effect.
        # Surface that instead of silently degrading (only inside a real repo,
        # so a non-git path stays quiet).
        if current_git_dir(repo_root) is not None:
            stderr.write(
                "spice git shadow inactive: no current branch (detached HEAD); "
                "native upstream config is in effect\n"
            )
            stderr.flush()
        return env
    config_path = write_shadow_config(repo_root, branch, stderr=stderr)
...
def write_shadow_config(
    repo_root: Path, branch: str, *, stderr: TextIO = sys.stderr
) -> Path | None:
    ...
    # /etc, Homebrew, Xcode layouts); git silently ignores the include when the
    # file is missing. The conventional /etc/gitconfig is the intended default
    # when discovery fails, but it is wrong on Homebrew/Xcode layouts, so log
    # the degradation rather than dropping system settings silently.
    resolved_system = real_system_config_path(repo_root)
    if resolved_system is None:
        system_config = "/etc/gitconfig"
        stderr.write(
            "spice git shadow: could not resolve git's system config path; "
            f"falling back to {system_config} (Homebrew/Xcode system git "
            "settings may be dropped)\n"
        )
        stderr.flush()
    else:
        system_config = resolved_system
    lines = [
