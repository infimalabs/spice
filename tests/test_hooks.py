...
def test_gates_only_initialization_installs_constitution_without_fleet_assets(tmp_path):
    ...
    assert rows == [
        "hook pre-commit -> .spice/hooks/pre-commit",
        "hook commit-msg -> .spice/hooks/commit-msg",
        "core.hooksPath=.spice/hooks",
        "state_ignore=.spice/.gitignore",
        "ready: git commit | spice dev pre-commit",
    ]
    assert (repo / ".spice" / ".gitignore").is_file()
...
def test_init_gates_cli_is_a_standalone_entry(tmp_path, monkeypatch):
    ...
    assert "hook commit-msg -> .spice/hooks/commit-msg" in result.stdout
    assert "ready: git commit | spice dev pre-commit" in result.stdout
    assert sorted(path.name for path in hooks_dir(repo).iterdir()) == [
        "commit-msg",
        "pre-commit",
    ]
    assert sorted(path.name for path in repo.iterdir()) == [".git", ".spice"]
...
def test_dev_install_hooks_previews_json_and_applies_shared_plan(
    ...
    parser = build_parser()
    plan = plan_hook_installation(repo)
    expected = initialization_plan_payload(plan)
...
def test_serve_web_typecheck_invokes_typescript_checkjs(tmp_path, monkeypatch):
    ...
    assert "spice/serve/static/app.lane-store.js" in argv
