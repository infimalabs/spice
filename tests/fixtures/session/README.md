...
| --- | --- | --- |
| `supervised_codex.jsonl` | Codex rollout events | Codex `task_started`/`task_complete` turn boundaries; bare and preamble `$spice` skill-invocation user messages; `custom_tool_call_output` list blocks; compaction-summary continuation packets; `<task-notification>` blocks; environment scaffold; human operator prose; `<1kCodex>` steering re-injection represented by assistant `ACK`/`NACK` headers; three compaction boundaries. |
