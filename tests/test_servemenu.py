...
def test_header_spice_menu_fast_mode_flame_pulses_behind_pepper():
    ...
    assert "font-size: 0.8em;" in icon_flame_rules
    assert "inset: 50% auto auto 50%;" in icon_flame_rules
    assert "z-index: 0;" in icon_flame_rules
    assert "filter:" not in icon_flame_rules
    assert "drop-shadow" not in icon_flame_rules
...
def test_index_branding_defaults_to_project_name_and_allows_explicit_override(
    ...
    assert (
        ...
        '"defaultLifetime": "Steer", "version":' in override_html
    )
...
def test_static_spice_menu_replaces_picker_lane():
    ...
    assert (
        ...
        "    ...change.transition.updates," in app_lanes
    )
    assert (
