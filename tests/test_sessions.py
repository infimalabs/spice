...
def test_briefing_payload_renders_filtered_briefing(tmp_path, monkeypatch):
    ...
    assert [(candidate.timestamp, candidate.text) for candidate in payload.asks] == [
        ("2026-01-01T00:00:04.000Z", "needle request")
    ]
    assert [(candidate.timestamp, candidate.text) for candidate in payload.finals] == [
        ("2026-01-01T00:00:04.000Z", "needle final")
    ]
