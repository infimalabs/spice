...
def test_work_tree_send_writes_inbox_and_returns_attachment_payload(
    ...
    monkeypatch.setattr(
        ...
        lambda _state, _target, thread_id: AutomaticLifecycleDecision(
            thread_id=thread_id,
            predecessor_actor="",
...
def test_team_historical_metrics_endpoint_projects_membership_intervals(
    ...
    set_time(240)
    store_merge_teams(state.team_store, split.team_id, team.team_id)
    ...
    set_time(360)
    store_split_team_back(state.team_store, team.team_id)
    ...
    set_time(480)
    store_remove_agent(state.team_store, team.team_id, "agent-b")
    state.team_store.record_agent_metric_delta("agent-b", message_timestamps=[540])

    set_time(600)
    store_close_team(state.team_store, split.team_id)
