...
class _TailStubDriver:
    """find_session_transcript stand-in pointing at one tmp transcript."""
...
def test_transcript_tail_scanner_captures_post_thinking_prose(tmp_path):
    # The claude CLI drops assistant text blocks that follow thinking blocks
    # from its stream-json stdout, so the tail must recover exactly this shape
    # from the transcript file with only the session announcement on stdout.
    from spice.agent.watchdog import JsonStdoutScanner, TranscriptTailScanner
    ...
    tail = TranscriptTailScanner(
        _TailStubDriver(transcript), scanner, poll_seconds=0.01
    )
    tail.observe_stdout_line('{"type":"system","subtype":"init","session_id":"abc123"}')
    line = json.dumps(
        {
            "type": "assistant",
            "message": {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "ACK 20260702T0Z: captured"},
    ...
    tail.close()
    assert captured == ["ACK 20260702T0Z: captured"]


def test_transcript_tail_scanner_skips_resumed_history(tmp_path):
    from spice.agent.watchdog import JsonStdoutScanner, TranscriptTailScanner
