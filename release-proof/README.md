...
`host-artifact-rehearsal`. When the container export's carried predecessor is
absent, the host rehearsal derives the same newest release below the declared
version from reachable tags, builds its wheel in private scratch space, and
checks the exact release identity and SHA-256 before installation. An existing
carry remains authoritative and is never regenerated around corruption. The
container appliance remains the only path that can emit the full Linux release
claim. Git-private records follow the checkout's `.git` directory or required
`gitdir:` pointer, so linked worktrees use their real Git directory instead of
treating the marker file as a directory.
