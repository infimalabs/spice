...
Superseded by the task phase contract in `docs/design/README.md`.
...
for larger no-worktree outputs. After `design` and `plan`, the missing primitive
...
is intentionally small: `design`, `plan`, `todo`, `verify`, and `review`.
...
| Research | Source-backed findings, options, citations | Design doc or sidecar notes; no production code | Needs durable citations and provenance |
| Discovery | Inventory of unknowns, related files, risks, task battery | Design doc plus follow-up tasks | Can turn into unbounded search |
...
| Trial | Measured experiment result | Design doc, benchmark log, sidecar artifact | Hard to reproduce if logs are not durable |
| High-end review | Structured findings, residual risk, follow-ups | Review note, design record, dependent tasks | Duplicates `review` unless it can carry richer artifacts |

All of these are legitimate work. Only `design` and `plan` have phase semantics;
...
or future architecture, turn it into a `design` phase task and write a reviewed
design record.
...
spawn or use a `design` phase task for the report, or use a future sidecar
...
Do not add more phase names beyond `design` and `plan`. Do not add a parallel
