...
| `spice/agent/lifecycle.py:666,669,670,768`; `spice/agent/wrap.py:202` | The supervisor, daemon reaper, and wrapped foreground child waits intentionally match the agent or command lifetime. Cleanup joins are capped at one second. | Lifetime-bound |
...
| `spice/agent/sidechannel.py:134,147,356,368,374`; `spice/agent/sidechannelnotify.py:60,61`; `spice/agent/wrap.py:619,620` | Server hello reads and replies, notifier connect/send, and agent-run connect/send all inherit explicit socket deadlines. Silent or wedged peers therefore release handler, publication, and launch paths. | Bounded, implemented under `RELIABI-1kCzJcnr` |
| `spice/agent/sidechannel.py:201,232,328,337,346`; `spice/agent/wrap.py:640,645` | Established stream selection, wake-socket reads/writes, and relayed reads intentionally stay open until parent exit, peer close, or server wake/stop. The bounded handshake timeout is cleared before this stream begins. | Lifetime-bound after connection |
| `spice/agent/sidechannel.py:94`; `spice/agent/wrap.py:600` | Side-channel thread joins are capped at one second. | Bounded |
