...
| Speech and serve-typecheck callers route through `spice/toolprocess.py` | Configurable speech, macOS `say`, and serve typechecking use named speech or typecheck deadlines with process-group cleanup. Worker coordination remains separately actionable. | Bounded child execution under `RELIABI-1kCzJtSj`; worker coordination: `RELIABI-1kCzJpcb` |
...
| `spice/serve/livebus.py:512,525,883` | Watcher activation, initial-payload release, and payload computation have no deadline. The single subscribe-completion worker can head-of-line block later subscribe batches. | Actionable: `RELIABI-1kCzJpcb` |
| `spice/serve/livebus.py:639` | Pending detached reads are joined through `concurrent.futures.wait`; the only production caller supplies the teardown timeout. | Bounded |
| `spice/serve/livebus.py:259,263,271,490,782,823,854` | Subscribe, follow-up, and metric workers block on queues until close sends sentinels; every cleanup join has the watcher join timeout. | Lifetime-bound workers with bounded cleanup |
| `spice/serve/livebus.py:1046`; bounded native loops at `spice/serve/livebus.py:1066-1080,1136-1200` | The kqueue and watchfiles waits poll with `LIVE_BUS_KQUEUE_CANCEL_TIMEOUT_S` and observe the subscription stop event; activation is signaled after the native watch is armed. | Bounded internally; callers still need the activation deadlines above |
