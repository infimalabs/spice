...
| `spice/process/git.py::run_git_command`; wrapper at `spice/tasks/gitsync.py::_run` | The shared git runner applies a configurable 120-second default; task `fetch` and `push` retain their tighter 30-second limit plus noninteractive SSH connect timeout. | Bounded, implemented under `GITSYNC-1kCzJQCl` and `RELIABI-1kCzJljJ` |
...
| `spice/serve/agentapi.py::<module>#RLock`; acquisitions in `spice/serve/agentapi.py::ensure_agent_for_available_work`, `spice/serve/agentapi.py::forget_available_work_observation`, and `spice/serve/agentapi.py::forget_available_work_observations` | The reentrant available-work decision mutex serializes capacity calculation, candidate selection, the guarded Taskwarrior claim, observation-cache cleanup, and agent startup so concurrent local lanes observe earlier successful starts before expanding again. Taskwarrior and Git commands inside the critical section have named deadlines, the claim-event SQLite write inherits its finite busy timeout, agent-ensure lock acquisition has its own deadline, and startup publication is deadline-bound. | Bounded by the task-backend, Git, SQLite, agent-ensure, and startup deadlines |
