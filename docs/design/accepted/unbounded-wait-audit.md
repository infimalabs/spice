...
| `spice/serve/agentapi.py::<module>#Lock`; acquisition in `spice/serve/agentapi.py::ensure_agent_for_available_work` | The available-work decision mutex serializes only the allocator snapshot and guarded Taskwarrior claim, so concurrent local lanes observe earlier successful claims and select distinct rows. Taskwarrior commands inside the critical section have the named 120-second deadline, and the claim-event SQLite write inherits its finite busy timeout; target Git reads and agent launch happen outside the mutex. | Bounded by the task-backend and SQLite deadlines |
