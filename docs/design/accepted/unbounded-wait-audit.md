...
| `spice/serve/agentapi.py::<module>#RLock`; acquisition in `spice/serve/agentapi.py::ensure_agent_for_available_work` | The process-wide reentrant mutex serializes each stopped Drain lane's capacity snapshot, queue-age read, guarded Taskwarrior claim, and agent startup, so the next concurrent inventory refresh observes the newly started lane before deciding whether backlog permits another expansion. The age is read off each candidate row's own `incepted` stamp inside that snapshot, so nothing outside the decision mutates shared state under this lock. Contending inventory refreshes can wait for the whole decision, but Taskwarrior and Git commands have named deadlines, SQLite writes inherit finite busy timeouts, agent-ensure lock acquisition is bounded, and supervisor startup publication has a named deadline. | Bounded by the task-backend, Git, SQLite, agent-ensure, and supervisor-startup deadlines |
